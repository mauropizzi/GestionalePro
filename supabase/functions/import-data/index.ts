...
// Importa le funzioni di utilità per la mappatura dei dati
import {
  getFieldValue,
  toString,
  toNumber,
  ...
  toDateString,
  isValidUuid
} from './utils/data-mapping.ts';

// Importa le funzioni di mappatura specifiche per ogni anagrafica
...
// Importa le funzioni per le operazioni sul database
...
// Mappatura dei tipi di anagrafica alle rispettive funzioni di mappatura
...
serve(async (req) => {
  ...
  try {
    ...
    for (const [rowIndex, row] of importData.entries()) {
      ...
      try {
        // Pass supabaseAdmin to the mapper if it's mapPuntoServizioData or any other mapper that needs it
        processedData = await mapper(row, supabaseAdmin); // All mappers now accept supabaseAdmin, even if they don't use it
