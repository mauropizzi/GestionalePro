...
async function mapPuntoServizioData(rowData: any, supabaseAdmin: any) {
  ...
  // Gestione di un potenziale errore di shift delle colonne per latitudine/longitudine
  if (latitude === null && longitude === null) {
    ...
    if (potentialShiftedLat !== null && potentialShiftedLon !== null && Math.abs(potentialShiftedLat) <= 90 && Math.abs(potentialShiftedLon) <= 180) {
      ...
      note = null; // Se i valori sono stati spostati, la nota originale potrebbe essere la latitudine
    }
  }
...
/** Mappa i nomi delle anagrafiche alle rispettive funzioni di mappatura. */
