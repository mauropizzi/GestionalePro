...
import { RUBRICA_FORNITORI_RECAPITO_TYPES } from "@/lib/constants";

const rubricaFornitoriContactFormSchema = z.object({
  tipo_recapito: z.enum(RUBRICA_FORNITORI_RECAPITO_TYPES as [string, ...string[]], {
...
export type RubricaFornitoriContactFormSchema = z.infer<typeof rubricaFornitoriContactFormSchema>;

interface RubricaFornitoriContactFormProps {
  onSubmit: (values: RubricaFornitoriContactFormSchema) => Promise<void>;
  isLoading: boolean;
  defaultValues?: Partial<RubricaFornitoriContactFormSchema>;
}

export function RubricaFornitoriContactForm({ onSubmit, isLoading, defaultValues }: RubricaFornitoriContactFormProps) {
  const form = useForm<RubricaFornitoriContactFormSchema>({
    resolver: zodResolver(rubricaFornitoriContactFormSchema),
    defaultValues: {
      tipo_recapito: defaultValues?.tipo_recapito || RUBRICA_FORNITORI_RECAPITO_TYPES[0],
          ...
          render={({ field }) => (
            <FormItem className="md:col-span-2">
              ...
              <Select onValueChange={field.onChange} value={field.value}>
                ...
                <SelectContent>
                  {RUBRICA_FORNITORI_RECAPITO_TYPES.map((type) => (
