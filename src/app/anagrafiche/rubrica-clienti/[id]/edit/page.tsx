...
export default function EditRubricaClientiContactPage() {
  const params = useParams();
  const contactId = params.id as string;
  const router = useRouter();

  const [clientName, setClientName] = useState<string | null>(null);
  const [contact, setContact] = useState<RubricaClientiContact | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);

  useEffect(() => {
    async function fetchContactAndClient() {
      if (!contactId) return;
      ...
      if (contactError) {
        ...
        router.push("/anagrafiche/clienti"); // Redirect to clients list if contact not found
        return;
      }
      ...
      if (clientError) {
        console.error("Supabase fetch client name error:", clientError);
        toast.error("Errore nel recupero del nome del cliente: " + clientError.message);
      } else if (clientData) {
        setClientName(clientData.ragione_sociale);
      }

      setIsLoading(false);
    }
    fetchContactAndClient();
  }, [contactId, router]);
  ...
  const handleUpdateContact = async (values: RubricaClientiContactFormSchema) => {
    ...
    if (error) {
      ...
      router.push(`/anagrafiche/clienti/${contact.client_id}/rubrica`);
    }
      ...
      <div className="container mx-auto py-6">
        <div className="flex items-center gap-4 mb-3">
          <Button variant="outline" size="icon" asChild>
            <Link href={`/anagrafiche/clienti/${contact.client_id}/rubrica`}>
              <ArrowLeft className="h-4 w-4" />
            </Link>
          </Button>
          <h1 className="text-2xl font-bold">Modifica Recapito per {clientName}</h1>
        </div>
