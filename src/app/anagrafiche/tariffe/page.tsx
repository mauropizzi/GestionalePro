...
import { ShieldAlert, Search, Loader2, Trash, Edit, PlusCircle, Tag } from "lucide-react";
    ...
    <DashboardLayout>
      <div className="container mx-auto py-8">
        ...
        <p className="text-lg text-muted-foreground mb-8">
          Gestisci le tariffe dei servizi.
        </p>
              ...
              {filteredTariffe.length === 0 && !loading ? (
                ...
                filteredTariffe.map((tariffa) => (
                  <TableRow key={tariffa.id}>
                    ...
                    <TableCell>{tariffa.importo ? `€ ${tariffa.importo.toFixed(2)}` : "N/A"}</TableCell>
                    <TableCell>
                      {tariffa.data_inizio_validita && tariffa.data_fine_validita
                        ? `${format(new Date(tariffa.data_inizio_validita), "PP", { locale: it })} - ${format(new Date(tariffa.data_fine_validita), "PP", { locale: it })}`
                        : "N/A"}
                    </TableCell>
