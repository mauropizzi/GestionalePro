...
import { RubricaFornitoriContactForm, RubricaFornitoriContactFormSchema } from "@/components/fornitori/rubrica-fornitori-contact-form.tsx";
...
export default function FornitoriRubricaPage() {
  const params = useParams();
  const fornitoreId = params.id as string;
  const { profile: currentUserProfile, isLoading: isSessionLoading } = useSession();

  const [contacts, setContacts] = useState<RubricaFornitoriContact[]>([]);
  const [loading, setLoading] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const [selectedContact, setSelectedContact] = useState<RubricaFornitoriContact | null>(null);
  ...
  useEffect(() => {
    if (!isSessionLoading && fornitoreId) {
      fetchContacts();
    }
  }, [isSessionLoading, fornitoreId]);
  ...
  const fetchContacts = async () => {
    ...
    if (error) {
      toast.error("Errore nel recupero dei recapiti della rubrica fornitori: " + error.message);
      setContacts([]);
    } else {
      setContacts(data || []);
    }
    ...
    <DashboardLayout>
      <div className="container mx-auto py-6">
        <div className="flex items-center gap-4 mb-3">
          ...
          <h1 className="text-2xl font-bold">Rubrica Fornitore</h1>
        </div>
        <p className="text-sm text-muted-foreground mb-4">
          Gestisci i recapiti per il fornitore con ID: <span className="font-semibold">{fornitoreId}</span>.
        </p>
          ...
          <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[600px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  {selectedContact ? "Apporta modifiche al recapito selezionato." : "Aggiungi un nuovo recapito alla rubrica di questo fornitore."}
                </DialogDescription>
              </DialogHeader>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 border-2 border-dashed rounded-lg p-6 text-center text-muted-foreground">
            ...
            <p className="text-lg font-medium">Nessun recapito trovato per questo fornitore.</p>
